...
const resolvers: Resolvers = {
  ...
  Mutation: {
    ...
    removePost: async (_, args, { orm }: Context) => {
      const post = await orm.postRepository.findOne(args.id, {
        relations: ["author"],
      });
      if (!post) {
    ...
    removeComment: async (_, args, { orm }: Context) => {
      const comment = await orm.commentRepository.findOne(args.id, {
    ...
    removeNotification: async (_, args, { orm }: Context) => {
      ...
      const notification = await notificationRepository.findOne(args.id, {
        relations: ["user"],
      });
      if (!notification) {
